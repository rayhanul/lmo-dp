...
parser.add_argument('--max_sentences', default=25, type=int, help='max sentences per step. Use a smaller value if your GPU runs out of memory')
parser.add_argument('--max_tokens', default=4000, type=int, help='max tokens per step. Use a smaller value if your GPU runs out of memory')
...
apdx += ' --sigma %f --clip %f '%(sigma, args.clip)
metric='accuracy'
n_classes=2
if(args.task == 'MNLI'):
    n_classes = 3
