...
def search_minimum_epsilon(SS, epsilon_threshold, demo_cnt=1000):
    # The searched condition is epsilon_threshold.
    cnt = 0
    searched_parameters = {}
    searched_parameters["overall_epsilon"] = epsilon_threshold
    search_min_epsilon = 0
    for lmo in take_Si(SS):
        ...
        if privacy_lmo==[]:
            ...
            overall_epsilon = get_log_epsilon(overall_epsilon, 64/60000 )
            if overall_epsilon < epsilon_threshold and np.isreal(rdp_lmo[int(opt_order-2)]):
                if overall_epsilon > search_min_epsilon and overall_epsilon < searched_parameters["overall_epsilon"] :
